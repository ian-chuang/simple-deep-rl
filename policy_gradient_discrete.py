...
def train():
    ...
    for i in range(16):
        ...
        while not done:
            with torch.no_grad():
                logits = agent.actor(torch.tensor(obs, dtype=torch.float32).to(device))
    ...
    if len(final_frames) == 16:
        ...
        plt.tight_layout()
        plt.savefig("outputs/policy_gradient_discrete_eval_grid.png")
